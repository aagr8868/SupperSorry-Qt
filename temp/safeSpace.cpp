...
SafeSpace::SafeSpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
SafeSpace::SafeSpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
void SafeSpace::MakeSpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
void SafeSpace::MakeSpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
QRectF SafeSpace::boundingRect() const
{
    return rec_;
}

// define the actual shape of the object
QPainterPath SafeSpace::shape() const
{
    QPainterPath path;
    path.addRect(rec_);
    return path;
...
void SafeSpace::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget);
