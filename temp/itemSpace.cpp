...
ItemSpace::ItemSpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
ItemSpace::ItemSpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
void ItemSpace::MakeSpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
void ItemSpace::MakeSpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
QRectF ItemSpace::boundingRect() const
{
    return rec_;
}

// define the actual shape of the object
QPainterPath ItemSpace::shape() const
{
    QPainterPath path;
    path.addRect(rec_);
    return path;
...
void ItemSpace::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget);
