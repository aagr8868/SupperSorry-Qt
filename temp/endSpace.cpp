...
EndSpace::EndSpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
EndSpace::EndSpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
void EndSpace::MakeSpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
void EndSpace::MakeSpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
QRectF EndSpace::boundingRect() const
{
    return rec_;
}

// define the actual shape of the object
QPainterPath EndSpace::shape() const
{
    QPainterPath path;
    path.addRect(rec_);
    return path;
...
void EndSpace::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget);
