...
EmptySpace::EmptySpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
EmptySpace::EmptySpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
void EmptySpace::MakeSpace(const QPointF center, const QColor color){
    center_ = center;
    color_ = color;
...
void EmptySpace::MakeSpace(const int x, const int y, const QColor color){
    center_.setX(x);
    center_.setY(y);
    color_ = color;
...
QRectF EmptySpace::boundingRect() const
{
    return rec_;
}

// define the actual shape of the object
QPainterPath EmptySpace::shape() const
{
    QPainterPath path;
    path.addRect(rec_);
    return path;
...
void EmptySpace::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    Q_UNUSED(widget);
